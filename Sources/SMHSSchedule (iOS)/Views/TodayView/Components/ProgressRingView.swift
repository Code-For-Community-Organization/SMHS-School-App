...
struct ProgressRingView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if let percent = scheduleDay?.currentPeriodPercentRemaining {
                Circle()
                    .trim(from: 0, to: CGFloat(percent))
                    .stroke(Color.primary, style: .init(lineWidth: 30, lineCap: .round))
                    .animation(.easeInOut)
                    .frame(width: 300, height: 300)
                    .rotationEffect(.degrees(-90))
            }
            VStack {
                if let periodNumber = scheduleDay?.currentPeriod?.periodNumber {
        ...
        .onAppear{
            self.countDown = scheduleDay?.currentPeriodTimeRemaining
        }
        .onReceive(timer){_ in
            if countDown != nil, countDown! > 1 {
                self.countDown! -= 1
            }
            else {
                self.countDown = scheduleDay?.currentPeriodPercentRemaining
            }
        }
    }
